...
def download_and_convert_to_wav(target_dir="./original_data"):
    ...
    if not download_with_retry(
        ...
        local_dir=temp_dir,
        allow_patterns=["Zip_files/AudioSet_SL/AudioSet_SL.z*", "json_files/AudioSet_SL/*.json"]
    ): return
    ...
    if not (target_path / "AudioSet_SL").exists():
        ...
        try:
            ...
            subprocess.run(["7z", "x", str(main_zip), f"-o{target_path}", "-y"], check=True)
    ...
    if total == 0:
        ...
        for i, flac_path in enumerate(flac_files):
            ...
            if wav_path.exists():
                if flac_path.exists(): os.remove(flac_path)
                continue
